...
Репозиторий проекта организован как экспериментальная среда для исследования алгоритмов паллетизации~\cite{project_repo}. Основные каталоги и файлы приведены в таблице~\ref{tab:repo-structure}.
...
Общий pipeline проекта показан на рис.~\ref{fig:architecture}. Сначала формируется или загружается входной JSON, затем один из решателей строит укладку, валидатор проверяет корректность и вычисляет метрики, после чего результаты сохраняются и могут быть визуализированы или агрегированы в benchmark-отчет.
...
Распределение задач по сценариям для каждого датасета показано на рис.~\ref{fig:dataset-composition}.
...
\centering
\small
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{0.31\textwidth}|c|X|}
\hline
