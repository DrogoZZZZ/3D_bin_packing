...
CONFIG_MULTI: Dict[str, Any] = {
    ...
    "beam_width":           16,       # шире чем в CONFIG (4) — лучшая плотность упаковки
    "beam_eps_limit":       40,
    "repair_queue_limit":   35,
...
# ──────────────────────────────────────────────────────────────────────────────
# Мульти-паллетный LNS
# ──────────────────────────────────────────────────────────────────────────────

def _multi_pallet_score(
    pallets_placed: List[List[Dict]],
    unplaced: List[Dict],
    pallet_spec: Dict,
    total_items: int,
    elapsed_ms: int,
) -> float:
    """Внутренняя оценка мульти-паллетного состояния для LNS.

    Штрафует лишние паллеты (0.08 за каждую сверх первой), чтобы
    стимулировать консолидацию. Отличается от evaluate_multi_pallet
    (которая используется для финального вывода).
    """
    if not pallets_placed:
        return 0.0

    pallet_vol = (
        pallet_spec["length_mm"] * pallet_spec["width_mm"] * pallet_spec["max_height_mm"]
    )
    total_placed = 0
    vol_utils: List[float] = []
    for pallet_boxes in pallets_placed:
        pvol = sum(
            (b["x_max"] - b["x_min"]) * (b["y_max"] - b["y_min"]) * (b["z_max"] - b["z_min"])
            for b in pallet_boxes
        )
        vol_utils.append(pvol / pallet_vol if pallet_vol > 0 else 0.0)
        total_placed += len(pallet_boxes)

    avg_vol = sum(vol_utils) / len(vol_utils)
    item_coverage = total_placed / total_items if total_items > 0 else 1.0
    pallet_penalty = 0.08 * (len(pallets_placed) - 1)
    ...
    return 0.50 * avg_vol + 0.30 * item_coverage - pallet_penalty + 0.10 * time_score


def _greedy_multi_pallet(
    items: List[Dict],
    pallet_spec: Dict,
    sort_key,
    all_heavy: bool,
) -> Tuple[List[List[Dict]], List[Dict]]:
    """Итеративная мульти-паллетная жадная упаковка.

    Возвращает (pallets_placed, truly_unplaced).
    pallets_placed — список паллет, каждая паллета — список placed box-dict'ов.
    """
    ...
    while remaining:
        ...
        placed, unplaced = greedy_with_order(
            ordered, pallet_spec, prefer_elevated_fragile=all_heavy
        )
        if not placed:
            # Тупик: ничего не помещается на чистую паллету
            break
        pallets_placed.append(placed)
        remaining = unplaced  # уже item-dict'ы с полем "reason"

    return pallets_placed, remaining


def destroy_multi_pallet(
    pallets_placed: List[List[Dict]],
    rng: random.Random,
    config: Dict,
    pallet_spec: Dict,
) -> Tuple[List[Dict], List[List[Dict]]]:
    """Destroy для мульти-паллетного LNS.

    Две стратегии:
      30% — Консолидация: удаляем все товары с самой разреженной паллеты.
      70% — Случайное кросс-паллетное: удаляем K товаров с 1–3 случайных паллет.

    Возвращает (removed_items_as_item_dicts, remaining_pallets).
    """
    if not pallets_placed:
        return [], []

    n_pallets = len(pallets_placed)
    removed_items: List[Dict] = []
    remaining_pallets: List[List[Dict]] = [list(p) for p in pallets_placed]

    pallet_vol = (
        pallet_spec["length_mm"] * pallet_spec["width_mm"] * pallet_spec["max_height_mm"]
    )

    if rng.random() < 0.30 and n_pallets >= 2:
        # ── Стратегия A: Консолидация ────────────────────────────────────────
        # Находим паллету с наименьшим объёмом размещённых коробов
        utils = []
        for i, pallet in enumerate(remaining_pallets):
            pvol = sum(
                (b["x_max"] - b["x_min"]) * (b["y_max"] - b["y_min"]) * (b["z_max"] - b["z_min"])
                for b in pallet
            )
            utils.append((pvol / pallet_vol if pallet_vol > 0 else 0.0, i))
        sparsest_idx = min(utils, key=lambda x: x[0])[1]

        removed_raw = remaining_pallets[sparsest_idx]
        removed_items = [_box_to_item(b) for b in removed_raw]
        remaining_pallets.pop(sparsest_idx)
    else:
        # ── Стратегия B: Случайное кросс-паллетное удаление ─────────────────
        n_hit = rng.randint(1, min(3, n_pallets))
        hit_indices = rng.sample(range(n_pallets), n_hit)

        # Выбираем только из не-full-restart эвристик
        eligible = [h for h in DESTROY_HEURISTICS if h != "full_restart"]
        for pidx in hit_indices:
            pallet = remaining_pallets[pidx]
            if not pallet:
                continue
            k = _compute_k(len(pallet), config)
            hname = rng.choice(eligible)
            # DESTROY_HEURISTICS принимает (placed, unplaced, k, rng)
            rem_p, removed_p = DESTROY_HEURISTICS[hname](pallet, [], k, rng)
            # Стабилизируем после удаления
            stable_p, cascade_p = stabilize_placed(rem_p)
            all_removed_p = removed_p + cascade_p
            remaining_pallets[pidx] = stable_p
            removed_items.extend(_box_to_item(b) for b in all_removed_p)

    # Убираем пустые паллеты
    remaining_pallets = [p for p in remaining_pallets if p]

    return removed_items, remaining_pallets


def repair_multi_pallet(
    pallets_placed: List[List[Dict]],
    items_to_repair: List[Dict],
    pallet_spec: Dict,
    total_items: int,
    ...
    """Repair для мульти-паллетного LNS.

    Фаза R1: добиваем существующие паллеты через beam_search_repair.
    Фаза R2: открываем новые паллеты жадным для оставшихся.

    Возвращает (new_pallets, still_unplaced).
    """
    if sort_key is None:
        all_heavy_local = all(it["weight"] > 2.0 for it in items_to_repair) if items_to_repair else False
        sort_key = lambda it: item_sort_key(it, all_heavy_local)

    q_limit = config.get("repair_queue_limit", 30)
    remaining = sorted(items_to_repair, key=sort_key)
    new_pallets: List[List[Dict]] = []

    # Фаза R1: заполняем существующие паллеты
    for pallet_boxes in pallets_placed:
        if not remaining:
            new_pallets.append(pallet_boxes)
            continue

        queue = remaining[:q_limit]
        repaired = beam_search_repair(
            pallet_boxes, queue, pallet_spec, total_items, config
        )
        new_pallets.append(repaired)

        fix_keys = {(b["sku_id"], b["instance_index"]) for b in pallet_boxes}
        repaired_keys = {(b["sku_id"], b["instance_index"]) for b in repaired}
        newly_placed_keys = repaired_keys - fix_keys

        remaining = [
            it for it in remaining
            if (it["sku_id"], it["instance_index"]) not in newly_placed_keys
        ]

    # Фаза R2: открываем новые паллеты для оставшихся
    while remaining:
        ordered = sorted(remaining, key=sort_key)
        placed, leftover = greedy_with_order(
            ordered, pallet_spec, prefer_elevated_fragile=all_heavy
        )
        if not placed:
            break
        new_pallets.append(placed)
        remaining = leftover

    # Убираем пустые паллеты
    new_pallets = [p for p in new_pallets if p]

    return new_pallets, remaining


def solve_lns_multi(request: Dict, config: Dict = CONFIG_MULTI) -> List[Dict]:
    """Мульти-паллетный LNS солвер.

    Возвращает List[Dict] — список паллет в том же формате, что solve_task_multi().
    Только последняя паллета содержит непустой список unplaced.
    """
    t_start = time.time()
    rng = random.Random(42)

    pallet_spec = request["pallet"]
    total_items_count = sum(b["quantity"] for b in request["boxes"])
    ...
    # ── Фаза 0: Мульти-старт жадный ──────────────────────────────────────────
    best_pallets: List[List[Dict]] = []
    best_unplaced: List[Dict] = []
    best_score = -1e9
    t_phase0_end = t_start + config.get("greedy_phase_s", 2.0)

    for _tag, _key in det_restarts:
        pallets, unplaced = _greedy_multi_pallet(items, pallet_spec, _key, all_heavy)
        t_ms = int((time.time() - t_start) * 1000)
        score = _multi_pallet_score(pallets, unplaced, pallet_spec, total_items_count, t_ms)
        if score > best_score:
            best_pallets, best_unplaced, best_score = pallets, unplaced, score
        if time.time() > t_phase0_end:
            break

    greedy_restarts = config.get("greedy_restarts", 50)
    for restart_i in range(1, greedy_restarts + 1):
        if time.time() > t_phase0_end:
            break
        rng_r = random.Random(restart_i * 31 + 7)
        items_r = _perturb_order(items, rng_r, _sort_key)
        pallets_r, unplaced_r = _greedy_multi_pallet(items_r, pallet_spec, _sort_key, all_heavy)
        t_ms = int((time.time() - t_start) * 1000)
        score_r = _multi_pallet_score(pallets_r, unplaced_r, pallet_spec, total_items_count, t_ms)
        if score_r > best_score:
            best_pallets, best_unplaced, best_score = pallets_r, unplaced_r, score_r

    if config["verbose"]:
        print(
            f"[LNS-MULTI] {request['task_id']} | "
            f"Phase0: pallets={len(best_pallets)}  "
            f"score={best_score:.4f}  "
            f"items={total_items_count}",
            file=sys.stderr,
        )
    ...
    cur_pallets = [list(p) for p in best_pallets]
    cur_unplaced = list(best_unplaced)
    ...
    while time.time() < t_deadline - SAFETY_MARGIN:
        ...
        removed_items, remaining_pallets = destroy_multi_pallet(
            cur_pallets, rng, config, pallet_spec
        )
        ...
        q_limit = config.get("repair_queue_limit", 30)
        all_to_repair = sorted(
            removed_items + cur_unplaced,
            key=lambda x: -(x["l"] * x["w"] * x["h"])
        )
        items_to_repair = all_to_repair[:q_limit]
        overflow_items  = all_to_repair[q_limit:]

        new_pallets, still_unplaced = repair_multi_pallet(
            remaining_pallets, items_to_repair, pallet_spec,
            total_items_count, config, rng, _sort_key, all_heavy
        )

        all_unplaced = still_unplaced + overflow_items

        t_ms = int((time.time() - t_start) * 1000)
        new_score = _multi_pallet_score(
            new_pallets, all_unplaced, pallet_spec, total_items_count, t_ms
        )

        # Критерий принятия SA
        accepted = False
        best_improved = new_score > best_score + 1e-6
        ...
        if config["accept_criterion"] == "sa":
            if new_score >= cur_score - 1e-9:
                ...
                prob = math.exp((new_score - cur_score) / sa_temp)
                accepted = rng.random() < prob
            sa_temp = max(sa_temp * config["sa_cooling_rate"], config["sa_min_temp"])
        else:
            accepted = best_improved
        ...
        if accepted:
            cur_pallets  = new_pallets
            cur_unplaced = all_unplaced
            cur_score    = new_score

        if best_improved:
            best_pallets  = new_pallets
            best_unplaced = all_unplaced
            best_score    = new_score
            no_improve    = 0
            if config["verbose"]:
                d = new_score - init_score
                print(
                    f"[LNS-MULTI]   iter={iteration:3d}  IMPROVED  "
                    f"score={new_score:.4f} ({d:+.4f} vs init)  "
                    f"pallets={len(new_pallets)}",
                    file=sys.stderr,
                )
        else:
            no_improve += 1

        # Early stop + ILS kicks
        if no_improve >= config["early_stop_patience"]:
            max_kicks = config.get("ils_kicks", 0)
            if n_kicks >= max_kicks or time.time() >= t_deadline - SAFETY_MARGIN:
                if config["verbose"]:
                    print(
                        f"[LNS-MULTI]   iter={iteration:3d}  early_stop "
                        f"(patience={config['early_stop_patience']}, kicks={n_kicks})",
                        file=sys.stderr,
                    )
                break
            ...
            no_improve = 0
            sa_temp    = config["sa_initial_temp"]
            all_kick = [_box_to_item(b) for p in best_pallets for b in p] + best_unplaced
            items_kick_r = _perturb_order(all_kick, rng, _sort_key)
            kick_pallets, kick_unplaced = _greedy_multi_pallet(
                items_kick_r, pallet_spec, _sort_key, all_heavy
            )
            _t = int((time.time() - t_start) * 1000)
            kick_score = _multi_pallet_score(
                kick_pallets, kick_unplaced, pallet_spec, total_items_count, _t
            )
            cur_pallets  = kick_pallets
            cur_unplaced = kick_unplaced
            cur_score    = kick_score
            if config["verbose"]:
                print(
                    f"[LNS-MULTI]   iter={iteration:3d}  ILS_kick={n_kicks}  "
                    f"restart_score={kick_score:.4f}  pallets={len(kick_pallets)}",
                    file=sys.stderr,
                )

    # ── Фаза 2: Post-fill ─────────────────────────────────────────────────────
    if best_unplaced:
        q_pf = config.get("repair_queue_limit", 30)
        unplaced_sorted = sorted(best_unplaced, key=_sort_key)[:q_pf]
        fill_pallets: List[List[Dict]] = []
        remaining_fill = list(unplaced_sorted)

        for pallet_boxes in best_pallets:
            if not remaining_fill:
                fill_pallets.append(pallet_boxes)
                continue
            filled = beam_search_repair(
                pallet_boxes, remaining_fill, pallet_spec, total_items_count, config
            )
            fill_pallets.append(filled)
            fix_keys    = {(b["sku_id"], b["instance_index"]) for b in pallet_boxes}
            filled_keys = {(b["sku_id"], b["instance_index"]) for b in filled}
            new_keys    = filled_keys - fix_keys
            remaining_fill = [
                it for it in remaining_fill
                if (it["sku_id"], it["instance_index"]) not in new_keys
            ]

        fill_ms = int((time.time() - t_start) * 1000)
        all_fill_placed_keys = {
            (b["sku_id"], b["instance_index"]) for p in fill_pallets for b in p
        }
        fill_unplaced = [
            it for it in best_unplaced
            if (it["sku_id"], it["instance_index"]) not in all_fill_placed_keys
        ]
        fill_score = _multi_pallet_score(
            fill_pallets, fill_unplaced, pallet_spec, total_items_count, fill_ms
        )
        if fill_score > best_score + 1e-6:
            best_pallets  = fill_pallets
            best_unplaced = fill_unplaced
            best_score    = fill_score
            if config["verbose"]:
                d = fill_score - init_score
                print(
                    f"[LNS-MULTI]   post_fill  score={fill_score:.4f} ({d:+.4f} vs init)  "
                    f"pallets={len(fill_pallets)}",
                    file=sys.stderr,
                )

    # ── Финал: строим вывод в формате solve_task_multi ────────────────────────
    total_elapsed_ms = int((time.time() - t_start) * 1000)

    if config["verbose"]:
        d = best_score - init_score
        print(
            f"[LNS-MULTI] FINAL: score={best_score:.4f} ({d:+.4f} vs init)  "
            f"pallets={len(best_pallets)}  iters={iteration}  total={total_elapsed_ms}ms",
            file=sys.stderr,
        )
    ...
    for pallet_boxes in best_pallets:
        ...
        results.append({
            ...
            "solve_time_ms":  total_elapsed_ms,
            "placements":     placements_out,
            "unplaced":       [],
        })
    ...
    # Непомещённые — в последнюю паллету
    if best_unplaced:
        unplaced_map: Dict[str, Dict] = {}
        for it in best_unplaced:
            rec = unplaced_map.setdefault(
                it["sku_id"], {"count": 0, "reason": it.get("reason", "no_space")}
            )
            rec["count"] += 1
...
def main() -> None:
    ...
    parser.add_argument(
        "--time", type=float, default=CONFIG["time_budget_s"],
        help=f"Бюджет времени в секундах (по умолчанию: {CONFIG['time_budget_s']})",
    )
    parser.add_argument(
        "--beam", type=int, default=CONFIG["beam_width"],
        help=f"Ширина луча (по умолчанию: {CONFIG['beam_width']})",
    )
    parser.add_argument("--quiet", action="store_true", help="Не выводить прогресс итераций")
    parser.add_argument("--hq", action="store_true",
                        help="Высококачественный режим (~28 с/задача, beam=8)")
    parser.add_argument("--multi", action="store_true",
                        help="Мульти-паллетный режим (solve_lns_multi, beam=8)")
    args = parser.parse_args()

    if args.multi:
        cfg = dict(CONFIG_MULTI)
        if args.time != CONFIG["time_budget_s"]:  # явно задан
            cfg["time_budget_s"] = args.time
        cfg["beam_width"] = args.beam if args.beam != CONFIG["beam_width"] else cfg["beam_width"]
        cfg["verbose"]    = not args.quiet
    else:
        cfg = dict(CONFIG_HQ if args.hq else CONFIG)
        cfg["time_budget_s"] = args.time
    ...
    if args.input:
        ...
        if args.multi:
            solutions = solve_lns_multi(request, cfg)
            ...
            total_unplaced = sum(u["quantity_unplaced"] for s in solutions for u in s["unplaced"])
            print(f"Решена '{request['task_id']}' -> {out_path}")
            print(f"  Паллеты:     {len(solutions)}")
            print(f"  Размещено:   {total_placed}")
            print(f"  Не размещено:{total_unplaced}")
            print(f"  Время:       {solutions[-1]['solve_time_ms']} мс")
        else:
            solution  = solve_lns(request, cfg)
            out_path  = args.output or PROJECT_ROOT / f"response_{request['task_id']}.json"
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(solution, f, indent=2, ensure_ascii=False)
            placed   = len(solution["placements"])
            unplaced = sum(u["quantity_unplaced"] for u in solution["unplaced"])
            print(f"Решена '{request['task_id']}' -> {out_path}")
            print(f"  Размещено:   {placed}")
            print(f"  Не размещено:{unplaced}")
            print(f"  Время:       {solution['solve_time_ms']} мс")
        ...
        if args.multi:
            ...
            for i, request in enumerate(dataset):
                sols = solve_lns_multi(request, cfg)
                all_solutions.append(sols)
                total_placed   = sum(len(s["placements"]) for s in sols)
                total_unplaced = sum(u["quantity_unplaced"] for s in sols for u in s["unplaced"])
                print(
                    f"[{i+1:4d}/{len(dataset)}] {request['task_id']:30s}  "
                    f"pallets={len(sols):2d}  placed={total_placed:4d}  "
                    f"unplaced={total_unplaced:4d}  time={sols[-1]['solve_time_ms']:5d}ms"
                )
            out_path = args.output or PROJECT_ROOT / "lns_multi_solution.json"
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(all_solutions, f, indent=2, ensure_ascii=False)
            print(f"\nСохранено {len(all_solutions)} решений → {out_path}")
        else:
            solutions = []
            for i, request in enumerate(dataset):
                sol      = solve_lns(request, cfg)
            ...
            out_path = args.output or PROJECT_ROOT / "lns_solution.json"
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(solutions, f, indent=2, ensure_ascii=False)
            print(f"\nСохранено {len(solutions)} решений → {out_path}")
